...
@php
    ...
    $imagePath = $menu->image_url
        ? (Str::startsWith($menu->image_url, 'http') ? $menu->image_url : Storage::url($menu->image_url))
        : null;

    $oldOptions = collect(old('options', $menu->options->map(fn ($option) => [
        'name' => $option->name,
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-semibold text-slate-800">Edit Menu</h1>
                <p class="text-sm text-slate-500">Perbarui detail menu serta atur addons yang tersedia untuk pelanggan.</p>
            </div>
        ...
        <form action="{{ route('menus.update', $menu) }}" method="POST" enctype="multipart/form-data" class="space-y-8 text-sm text-slate-600">
            @csrf
            @method('PUT')
            ...
            <section class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm shadow-slate-900/5">
                ...
                <p class="text-xs text-slate-500">Pastikan data menu akurat agar pelanggan tidak keliru saat memesan.</p>
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-2">
                    ...
                    <label class="space-y-2">
                        ...
                        <input type="number" name="price" min="0" step="100" value="{{ old('price', $menu->price) }}" required
                            class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                    </label>
                    <label class="space-y-2">
                        <span class="font-semibold text-slate-700">Stok</span>
                        <input type="number" name="stock" min="0" value="{{ old('stock', $menu->stock) }}" required
                            class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                    </label>
                    <label class="md:col-span-2 space-y-2">
                        <span class="font-semibold text-slate-700">Deskripsi</span>
                        <textarea name="description" rows="4"
                            class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">{{ old('description', $menu->description) }}</textarea>
                    </label>
                    <div class="space-y-3">
                        <label class="space-y-2">
                            <span class="font-semibold text-slate-700">Unggah Foto Baru</span>
                            ...
                            <span class="block text-xs text-slate-400">Biarkan kosong jika tidak ingin mengubah foto.</span>
                        </label>
                        @if ($imagePath)
                            <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                                <p class="text-xs font-semibold text-slate-500">Pratinjau saat ini</p>
                                <img src="{{ $imagePath }}" alt="{{ $menu->name }}" class="mt-3 h-40 w-full rounded-xl object-cover">
                            </div>
                        @endif
                    </div>
            ...
            <section class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm shadow-slate-900/5">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Sesuaikan pilihan tambahan yang dapat dipilih pelanggan.</p>
                    </div>
                ...
                <div id="addon-container" data-next-index="{{ $oldOptions->count() }}" class="mt-6 space-y-4">
                    @forelse ($oldOptions as $idx => $option)
                        <div class="addon-row grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 md:grid-cols-[1.4fr_minmax(120px,0.6fr)_auto]">
                            <label class="space-y-1">
                                <span class="text-xs font-semibold text-slate-500">Nama addon</span>
                                <input type="text" name="options[{{ $idx }}][name]" value="{{ $option['name'] ?? '' }}" required
                            ...
                            <label class="space-y-1">
                                ...
                                <input type="number" name="options[{{ $idx }}][extra_price]" min="0" step="100" value="{{ $option['extra_price'] ?? 0 }}"
                                    class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                            </label>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                function ensureEmptyState() {
                    const hasRow = container.querySelectorAll('.addon-row').length > 0;
                    ...
                    if (hasRow) {
                        ...
                        empty.className = 'empty-state text-xs text-slate-400';
                        empty.textContent = 'Belum ada addons. Klik tombol di atas untuk menambah.';
                ...
                function buildRow(index) {
                    ...
                    bindRemove(wrapper.querySelector('.remove-addon'), wrapper);

                    return wrapper;
                }

                addButton?.addEventListener('click', () => {
                    const next = Number(container.dataset.nextIndex ?? 0);
                    const row = buildRow(next);
                    container.insertBefore(row, container.querySelector('.empty-state'));
                    container.dataset.nextIndex = next + 1;
                    ensureEmptyState();
                });

                container.querySelectorAll('.addon-row').forEach((row) => {
                    const remove = row.querySelector('.remove-addon');
                    if (remove) {
